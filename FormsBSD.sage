...
for Itemp in range(1,1001):
    ...
    for a in L:
        ...
        #These H bounds translate to c bounds under the identity H = 8ac - 3b^2

        #The bounds on b are -2|a| < b leq 2|a|, found in Birch and Swinnerton-Dyer
        ...
        for b in range(-2*abs(a) + 1, 2*abs(a)+1):
            ...
            for c in range(int(math.ceil(cmin)),int(math.floor(cmax))+1):
                ...
                #Compute R using the seminvariant syzygy
...
g = open("FormStatisticsBSD.tex","w")
...
os.system("pdflatex FormsBSD.tex")
os.system("pdflatex FormStatisticsBSD.tex")
